...
export function buildAppointmentSystemNote({ clinicName, clinicPhone, professionalName, professionalPhone }) {
  ...
  const clinicContact = normalizeContactPhone(clinicPhone);
  const phone = clinicContact || normalizeContactPhone(professionalPhone);
  const contact = phone
    ? `${phone === clinicContact ? clinic : (professionalName || 'el profesional')}: https://wa.me/${phone}`
