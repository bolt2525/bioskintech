 ...
 *
 * @param {import('@vercel/node').VercelRequest} req
 * @returns {Promise<{valid: boolean, username?: string, role?: string, clinic_id?: number|null, effective_clinic_id?: number|null}>}
 */
export async function authenticateRequest(req) {
  ...
  try {
    ...
    if (role === 'master_admin') {
      ...
      // UUID string — validación de formato básico antes de usar
      if (targetHeader && /^[0-9a-f-]{36}$/i.test(targetHeader)) {
