...
export async function listWhatsAppContacts(search = '', limit = 100) {
  ...
  const result = await sql`
    SELECT c.id, c.phone, c.name, c.created_at, c.last_message_at,
           c.clinic_id, cl.name AS clinic_name,
    ...
    LEFT JOIN clinic_users cu ON cu.clinic_id = c.clinic_id
      AND cu.is_active = true
      AND regexp_replace(COALESCE(cu.phone, ''), '\\D', '', 'g') = c.phone
    LEFT JOIN LATERAL (
  ...
  return result.rows.map(row => {
    ...
    return {
      ...row,
      category: isSystemStaff ? 'staff_sistema' : isClinicUser ? 'usuario_clinico' : row.clinic_id ? 'paciente_clinica' : 'sin_clasificar',
      category_label: isSystemStaff ? 'Staff del sistema' : isClinicUser ? 'Usuario clínico' : row.clinic_name ? `Paciente · ${row.clinic_name}` : 'Sin clasificar',
    };
  });
}
