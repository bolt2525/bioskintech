...
export async function initClinicalDatabase() {
  ...
  try {
    ...
    // ── Fotos clínicas (almacenadas en Cloudflare R2) ─────────────────────
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_clinical_photos_record ON clinical_photos(record_id)`);
