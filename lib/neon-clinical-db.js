...
export async function initClinicalDatabase() {
  ...
  try {
    ...
    const consultationMigrations = [
      ...
      // Constraint RUT multi-tenant: global UNIQUE → UNIQUE por clínica
      'ALTER TABLE patients DROP CONSTRAINT IF EXISTS patients_rut_key',
      ...
      // Columna tenant en patients si no existe (migración desde registros legacy)
      'ALTER TABLE patients ADD COLUMN IF NOT EXISTS created_by_user_id INTEGER',
    ];
