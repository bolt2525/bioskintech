...
export function getAppPool() {
  ...
  if (!url) { console.error('❌ NEON_APP_URL no configurada. Se rechaza el pool clínico para no omitir RLS.'); return null; }
  try { appPoolInstance = makePool(url); return appPoolInstance; }
  catch (e) { console.error('❌ Error creando pool app:', e); return null; }
}
  ...
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS prescriptions (
        ...
        items           JSONB,
        notes           TEXT,
        prescription_mode VARCHAR(20) NOT NULL DEFAULT 'routine',
        validity_type   VARCHAR(30),
        valid_until     DATE,
        regulatory_snapshot JSONB NOT NULL DEFAULT '{}'
      )
    `);
    await pool.query("ALTER TABLE prescriptions ADD COLUMN IF NOT EXISTS prescription_mode VARCHAR(20) NOT NULL DEFAULT 'routine'");
    await pool.query("ALTER TABLE prescriptions ADD COLUMN IF NOT EXISTS validity_type VARCHAR(30)");
    await pool.query("ALTER TABLE prescriptions ADD COLUMN IF NOT EXISTS valid_until DATE");
    await pool.query("ALTER TABLE prescriptions ADD COLUMN IF NOT EXISTS regulatory_snapshot JSONB NOT NULL DEFAULT '{}'::jsonb");
    ...
    // ── Finanzas externas — aislamiento por clínica ──────────────────────
    await pool.query(`
      CREATE TABLE IF NOT EXISTS external_finance_records (
        ...
        doctor_fees JSONB DEFAULT '[]',
        expenses NUMERIC(12,2) DEFAULT 0,
        additional_income NUMERIC(12,2) DEFAULT 0,
        net_income_juan_pablo NUMERIC(12,2) DEFAULT 0,
        raw_note TEXT,
    ...
    `);
    const appRole = await pool.query(`SELECT 1 FROM pg_roles WHERE rolname = 'bioskin_app'`);
    if (appRole.rows.length) {
      await pool.query('ALTER TABLE external_finance_records ENABLE ROW LEVEL SECURITY');
      await pool.query('ALTER TABLE external_finance_records FORCE ROW LEVEL SECURITY');
      ...
      for (const policy of ['p_select', 'p_insert', 'p_update', 'p_delete']) {
        await pool.query(`DROP POLICY IF EXISTS ${policy} ON external_finance_records`);
      }
      await pool.query(`CREATE POLICY p_select ON external_finance_records FOR SELECT TO bioskin_app USING (${tenantExpr})`);
      await pool.query(`CREATE POLICY p_insert ON external_finance_records FOR INSERT TO bioskin_app WITH CHECK (${tenantExpr})`);
      await pool.query(`CREATE POLICY p_update ON external_finance_records FOR UPDATE TO bioskin_app USING (${tenantExpr}) WITH CHECK (${tenantExpr})`);
      await pool.query(`CREATE POLICY p_delete ON external_finance_records FOR DELETE TO bioskin_app USING (${tenantExpr})`);
    }
    ...
    const migrations = [
      ...
      'ALTER TABLE external_finance_records ADD COLUMN IF NOT EXISTS clinic_id UUID',
      'CREATE INDEX IF NOT EXISTS idx_external_finance_records_clinic ON external_finance_records(clinic_id)',
      ...
      // Tratamientos: equipment_used ahora admite lista de equipos separados por coma (multi-aparatología por sesión)
      'ALTER TABLE treatments ALTER COLUMN equipment_used TYPE TEXT',
    ];
