...
- ✅ 2026-09-17 Plantilla `resumen_diario_agenda` aprobada por Meta con 2 variables (`fecha`, `resumen`) y botón de respuesta rápida "Confirmado". Actualizado el envío en `api/whatsapp-chatbot.js` para pasar ambos parámetros; el resumen se aplana (sin saltos de línea, separador ` · `) antes de enviarse como parámetro de plantilla porque Meta rechaza valores con saltos de línea en el Cloud API (el modo texto libre dentro de la ventana de 24h conserva el formato con líneas).
