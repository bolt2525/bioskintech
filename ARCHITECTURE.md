...
El flujo real de confirmación del paciente ya no dispara WhatsApp en el momento del booking: la confirmación inmediata sigue siendo por correo electrónico, mientras que `sendPatientAppointmentReminders()` revisa los eventos del calendario del día siguiente, extrae `Teléfono:` del evento con `parseAppointmentEvent()` y envía el mensaje de recordatorio usando la plantilla autorizada (`WHATSAPP_TEMPLATE_APPOINTMENT` / `..._LANG`) sin duplicados, marcando el envío en `extendedProperties.private.bioskinReminderSent` con la fecha local `AAAA-MM-DD` del evento. Esto mantiene el mismo patrón de plantillas aprobado por Meta y evita reaprobar una nueva plantilla solo por cambiar el momento del envío.
