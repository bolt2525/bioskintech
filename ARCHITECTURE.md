...
El agendamiento público se activa con `clinic_users.public_booking_enabled` y usa rutas formadas por el slug de la clínica y el `username` del profesional. La reserva no requiere sesión administrativa, pero sí verifica que el recurso exista, esté activo y esté autorizado, aplica `rate limit` por IP, rechaza inputs honeypot y exige Turnstile cuando la secret `TURNSTILE_SECRET` está activa. El evento se inserta con el mismo OAuth del profesional y la misma validación de solapamiento que el flujo interno. La consulta de perfil público se expone antes del gate de sesión para evitar el error de “No autenticado o sesión expirada” al abrir el enlace público. La publicación es parcial: solo se entregan al paciente los tratamientos cuya duración guardada está entre 30 y 180 minutos; el backend vuelve a resolver esa duración desde Neon y rechaza cualquier servicio no publicado, aunque el cliente manipule la solicitud. Para activar el enlace basta con tener al menos un tratamiento publicable.
