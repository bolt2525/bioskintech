...
- `lib/whatsapp-service.js` envía mensajes vía WhatsApp Cloud API (Graph API) y falla cerrado si `WHATSAPP_TOKEN`/`WHATSAPP_PHONE_NUMBER_ID` no están configuradas; `api/sendEmail.js` lo invoca en el agendamiento solo si `clinic_settings.notificaciones.whatsapp_enabled` es `true`, sin bloquear el flujo de calendario/correo si falla. `whatsapp_enabled` es opt-in y solo lo edita `master_admin` (pestaña "Notificaciones" del modal de ajustes de clínica); si nunca se activa no queda ningún rastro en `whatsapp_messages` porque el código nunca intenta el envío.
- Meta exige una plantilla de mensaje (HSM) aprobada para notificaciones iniciadas por el negocio fuera de la ventana de servicio al cliente de 24h (es decir, cuando el paciente no le ha escrito antes al número). `lib/whatsapp-crm.js#isWithinCustomerServiceWindow()` revisa si hubo un mensaje entrante del contacto en las últimas 24h; si sí, se envía texto libre con `sendWhatsAppText()`, si no, se usa `sendWhatsAppTemplate()` con el nombre/idioma configurados en `WHATSAPP_TEMPLATE_APPOINTMENT`/`WHATSAPP_TEMPLATE_APPOINTMENT_LANG`. Sin esa plantilla aprobada en Meta Business Manager y sin esas env vars, el envío fuera de ventana falla con un mensaje de error explícito registrado en `whatsapp_messages` (status `fallido`).
