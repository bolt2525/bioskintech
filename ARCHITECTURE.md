...
La reserva no requiere sesión administrativa, pero el servidor verifica que el recurso pertenezca al titular, esté activo y solo acepte ayudantes cuando el modo multiusuario esté habilitado. Cada evento se etiqueta con su `resourceId`; los conflictos se comprueban exclusivamente contra ese recurso, lo que permite citas paralelas de profesionales distintos. `GET /api/public-booking` devuelve los slots libres del día usando la duración configurada para el tratamiento publicado, la jornada del recurso y los eventos actuales de Google Calendar; `POST` vuelve a resolver esa duración y valida el conflicto antes de insertar. La cita debe caber por completo en el horario general o en el horario específico del ayudante. También se aplican `rate limit` por IP, honeypot y Turnstile; en producción, la ausencia de `TURNSTILE_SECRET` falla cerrado. La CSP autoriza explícitamente los scripts, conexiones y frames de Cloudflare Turnstile.
