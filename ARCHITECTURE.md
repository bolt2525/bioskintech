...
- Opción 3 del menú de clínica: consulta de agenda de un día específico vía máquina de estados (`agendaDateStateByPhone`, un solo stage `awaitingDate`). `parseFlexibleDate()` acepta "hoy"/"mañana"/`DD/MM/AAAA`/`DD-MM-AAAA`/`AAAA-MM-DD` y valida que la fecha exista realmente (rechaza desbordes tipo 31/02 en vez de "normalizarlos" silenciosamente). `listAppointmentsForDate()` generaliza la consulta a Google Calendar antes usada solo para "hoy".
...
- `lib/whatsapp-service.js` envía mensajes vía WhatsApp Cloud API (Graph API) y falla cerrado si `WHATSAPP_TOKEN`/`WHATSAPP_PHONE_NUMBER_ID` no están configuradas; `api/sendEmail.js` lo invoca en el agendamiento solo si `clinic_settings.notificaciones.whatsapp_enabled` es `true`, sin bloquear el flujo de calendario/correo si falla. `whatsapp_enabled` es opt-in y lo edita tanto `clinic_admin` (pestaña Agenda de sus propios Ajustes, dashboard de usuario) como `master_admin` (pestaña "Notificaciones" del modal de ajustes de clínica); si nunca se activa no queda ningún rastro en `whatsapp_messages` porque el código nunca intenta el envío.
