...
El CRM master clasifica contactos verificables como staff del sistema, usuario clínico, paciente asociado a clínica o sin clasificar. La relación se resuelve por teléfono canónico contra `clinic_users` y `patients`, incluso cuando `whatsapp_contacts.clinic_id` histórico está vacío; si la agenda y la ficha clínica discrepan, se muestra la clínica real de la ficha en lugar de inferirla. Los nuevos envíos de confirmación y resumen pasan `clinicId` al auditor para conservar la clínica de origen. La lista y el historial tienen scroll independiente y cada conversación abre posicionada en el mensaje más reciente.

El modal de agendamiento busca pacientes por nombre dentro de la clínica efectiva de la sesión mediante `listPatients`; la selección rellena nombre, correo y teléfono como valores editables, sin asumir que un teléfono global identifica una única clínica. Antes de confirmar muestra el número que recibirá WhatsApp solo cuando `whatsapp_bot_enabled` (autorización de Master Admin) y `whatsapp_confirm_enabled` (configuración del usuario) están activos; el backend vuelve a aplicar ambos controles.
